...
arma::vec mean_piece_old = gamma_star - A21_old*delta1 - A22_old*delta2;

double second = -0.50*dot(mean_piece_old, mean_piece_old) - 
                0.50*(1/sigma2_A)*(A22_trans_old*A22_trans_old);

/*First*/
double A22_trans = R::rnorm(A22_trans_old, 
                            sqrt(metrop_var_A22_trans));
double A22 = exp(A22_trans);
arma::vec mean_piece = gamma_star - A21_old*delta1 - A22*delta2;

double first = -0.50*dot(mean_piece, mean_piece) - 
               0.50*(1/sigma2_A)*(A22_trans*A22_trans);
